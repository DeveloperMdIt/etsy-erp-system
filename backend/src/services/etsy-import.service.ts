...
export class EtsyImportService {
    ...
    private async processApiOrder(tenantId: string, userId: string, receipt: any) {
        ...
        if (!finalFirstLine && !finalCity) {
            finalFirstLine = 'Unknown (Etsy Privacy)';
            finalCity = 'Unknown';
            finalZip = '00000';
            reviewNote = '⚠️ Warnung: Keine Adressdaten von Etsy erhalten (Datenschutz). Bitte im Etsy-Backend prüfen oder CSV importieren.';
        ...
        const orderData = {
            ...
            externalOrderId: `${receipt.receipt_id}`,
            platform: 'ETSY',
        ...
        const createdOrder = await prisma.order.create({
            data: {
                ...
                notes: orderData.notes // Use the notes we built earlier
            }
        });
